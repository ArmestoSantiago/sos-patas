...
export const mapData = (data: React.FormEvent<HTMLFormElement>) => {
  ...
  const condition = elements.petCondition.value === PetsCondition.HEALTHY ? PetsCondition.HEALTHY : elements.petCondition.value === PetsCondition.WOUNDED ? PetsCondition.WOUNDED : PetsCondition.CRITICAL;
